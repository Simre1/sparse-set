...
module Data.SparseSet.Storable2 where
...
unsafeReadDense :: forall a. Storable a => SparseSetStorable a -> DenseArray a -> Int -> IO (Word32, a)
unsafeReadDense _ (DenseArray ptr) i = do
  let elementPtr = ptr `plusPtr` (i * (sizeOf' @Word32 + sizeOf' @a))
  e <- peek $ castPtr elementPtr
  v <- peek $ castPtr (elementPtr `plusPtr` sizeOf' @Word32)
  pure (e, v)
{-# INLINE unsafeReadDense #-}

unsafeReadDenseValue :: forall a. Storable a => SparseSetStorable a -> DenseArray a -> Int -> IO a
...
unsafeReadDenseKey :: forall a. Storable a => SparseSetStorable a -> DenseArray a -> Int -> IO Word32
...
unsafeWriteDense :: forall a. Storable a => SparseSetStorable a -> DenseArray a -> Int -> (Word32, a) -> IO ()
unsafeWriteDense _ (DenseArray ptr) i (e, v) = do
  let elementPtr = ptr `plusPtr` (i * (sizeOf' @Word32 + sizeOf' @a))
  poke (castPtr elementPtr) e
  poke (castPtr (elementPtr `plusPtr` sizeOf' @Word32)) v
{-# INLINE unsafeWriteDense #-}

unsafeWriteDenseValue :: forall a. Storable a => SparseSetStorable a -> DenseArray a -> Int -> a -> IO ()
...
visualize :: Storable a => MonadIO m => SparseSetStorable a -> m ()
