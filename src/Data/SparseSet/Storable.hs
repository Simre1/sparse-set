...
contains (SparseSetStorable sparse _ _ _) i = liftIO $ do
...
size (SparseSetStorable _ _ _ sizeRef) = liftIO $ readIORef sizeRef
{-# INLINE size #-}
...
remove (SparseSetStorable sparse entities dense sizeRef) i = liftIO $ do
  index <- VPM.unsafeRead sparse (fromIntegral i)
  if index == maxBound
    then pure ()
    else do
...
iterate (SparseSetStorable _ entities dense sizeRef) f = do
...
visualize (SparseSetStorable sparse entities sense sizeRef) = liftIO $ do
