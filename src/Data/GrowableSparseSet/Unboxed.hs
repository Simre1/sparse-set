...
instance NFData (SparseSetUnboxed a) where
  rnf (SparseSetUnboxed {}) = ()

type ElementConstraint a = VM.Unbox a :: Constraint
...
create sparseSize denseSize = liftIO $ do
  ...
  let !size = 0
  SparseSetUnboxed sparse entities dense <$> newIORef IM.empty <*> newIORef size
{-# INLINE create #-}
...
insert (SparseSetUnboxed sparse entities dense backupRef sizeRef) i a = liftIO $ do
...
unsafeContains :: (MonadIO m) => SparseSetUnboxed a -> Word32 -> m Bool
unsafeContains (SparseSetUnboxed sparse _ _ _ _) i = liftIO $ do
...
size (SparseSetUnboxed _ _ _ backupRef sizeRef) =
  liftIO $
...
lookup (SparseSetUnboxed sparse _ dense backupRef _) i = liftIO $ do
  index <- VPM.unsafeRead sparse (fromIntegral i)
  if notFreeOrInBackup index
...
unsafeLookup (SparseSetUnboxed sparse _ dense backupRef _) i = liftIO $ do
  index <- VPM.unsafeRead sparse (fromIntegral i)
  if index /= inBackup
...
remove (SparseSetUnboxed sparse entities dense backupRef sizeRef) i = liftIO $ do
  index <- VPM.unsafeRead sparse (fromIntegral i)
  if notFreeOrInBackup index
    then do
...
iterate :: (ElementConstraint a, MonadIO m) => SparseSetUnboxed a -> (Word32 -> a -> m ()) -> m ()
iterate (SparseSetUnboxed _ entities dense backupRef sizeRef) f = do
...
growDenseIfNecessary :: (ElementConstraint a, MonadIO m) => SparseSetUnboxed a -> m (SparseSetUnboxed a)
growDenseIfNecessary oldSparseSet@(SparseSetUnboxed sparse entities dense backupRef sizeRef) = liftIO $ do
  ...
  if IM.size backup == 0
    then pure oldSparseSet
      ...
      let newSparseSet = SparseSetUnboxed sparse newEntities newDense backupRef sizeRef
...
visualize (SparseSetUnboxed sparse entities dense backupRef sizeRef) = liftIO $ do
